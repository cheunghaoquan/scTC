...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', default='results/scTC_pancreas/',
                        help='directory to save model weights during the training stage')
    parser.add_argument('--ae_weight_file', default='AE_weights_pancreas.pth.tar',
                        help='file name to save model weights after the pretraining stage')
    parser.add_argument('--final_latent_file', default='final_latent_file_pancreas.txt',
                        help='file name to save final latent representations')
    parser.add_argument('--predict_label_file', default='pred_labels_pancreas.txt',
    ...
    x = pd.read_csv("./data/pancreas.csv",header= None)
    x = np.array(x)
    y = pd.read_csv("./data/pancreas_label.csv",header= None)
    ...
    y_pred, _, _, _, _ = model.fit(X=adata.X, X_raw=adata.raw.X, size_factor=adata.obs.size_factors,
                                   n_clusters=args.n_clusters, init_centroid=None, anchor=anchor, positive=positive,
                                   negative=negative,
