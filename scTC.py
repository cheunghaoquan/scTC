...
class scTC(nn.Module):
    def __init__(self, input_dim, z_dim, encodeLayer=[], decodeLayer=[], activation="relu", sigma=1., alpha=1., gamma=1., device="cuda"):
        super(scTC, self).__init__()
        self.z_dim = z_dim
    ...
    def fit(self, X, X_raw, size_factor, n_clusters, anchor, positive, negative,
        ...
        for epoch in range(num_epochs):
            if epoch % update_interval == 0:
                ...
                print('\t\tClustering %d: NMI= %.4f, ARI= %.4f,ACC= %.4f' % (epoch, nmi, ari, acc))
            ...
            print("Epoch%3d: Clustering Loss: %.8f ZINB Loss: %.8f" % (
                epoch + 1, cluster_loss_val / num, recon_loss_val / num), end=' ')
            ...
            if epoch % update_interval == 0:
                ...
                for tri_batch_idx in range(tri_num_batch):
                    ...
                    target1 = Variable(pbatch1).to(self.device)
                    target2 = Variable(pbatch2).to(self.device)
                    target3 = Variable(pbatch3).to(self.device)
            ...
            train_loss = cluster_loss_val * self.gamma + recon_loss_val + triplet_loss_val
            print("\t\t*Total Loss: %.8f" % (train_loss / num))
